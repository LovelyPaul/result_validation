...
**세 번째 에이전트 = 문서 팩트체크**(`dials/doc-factcheck.json` + `ev/doc-factcheck/`)는 코드
리뷰의 교훈(좌표≠근거)을 처음부터 반영해 **첫 판에 거부율 100%**로 붙었다. 계약서·정책 문서의
수치·인용이 원본 조항(제N조·§)에 실재하는지 검증한다. 세 도메인(논문·코드·문서)이 **같은 코어
무수정**으로 도는 것 — 이것이 "코어 한 번 + 다이얼로 도메인 교체"가 사실임의 증명이다.

## 의미 채널 (G1+) — 붕 뜬 근거를 귀띔 (판정 아님)

`semantic_core.py`는 grep이 못 잡는 주장 — **원문 어디와도 의미적으로 안 닮은 근거** — 를
사람에게 WARN으로 귀띔한다. grep(정밀 FAIL)과 상보적: 방향이 반대다(요약 근거→원문 대응 부재).

★ **판정 권한 0**: FAIL도 exit code도 절대 안 낸다. 결정론 grep만이 거부 권한을 갖고,
CI 거부율은 이 채널과 무관하게 계산된다. 임계를 바꿔도 게이트 통과/실패는 안 바뀐다.
CI에는 **정보 전용·비차단 스텝**으로만 들어간다(로그에 남되 빌드 결정 불간섭).

```bash
python3 semantic_core.py --dir ev/research-paper --dial dials/research-paper.json \
  --corpus ev/research-paper/corpus.sample.json [--sim-threshold 0.2]
python3 semantic_core.py --self-test
```

**정직한 한계**(stdlib 문자 n-gram 코사인 — 실측): 완전히 붕 뜬 주장(무관·표면 왜곡,
sim ~0.1)은 잘 잡지만, 충실한 의역(~0.39)·부분 인용 왜곡(~0.39)은 겹쳐 구분 못 한다.
그래서 **낮은 임계 미만만 WARN**(저재현·저오탐)한다 — 정상 의역을 오탐하지 않는 게 우선.
진짜 의미 이해(비교 방향 뒤집기 등 정밀 판별)는 임베딩/NLI 몫(로드맵). 오인용 확정은
여전히 grep이 한다. 의미 채널은 "이 주장 원문 어디에도 안 닮았는데요?" 하는 보조 귀띔이다.
...
1. `dials/<도메인>.json`을 만든다 — 위 스키마대로 그 도메인의 근거 규칙을 채운다.
   ...
   - **문서 팩트체크** (구현됨 — `dials/doc-factcheck.json` + `ev/doc-factcheck/`):
     `evidence_section` = `## 근거`, `cite_pattern` = 조항(제N조)·절(§)·페이지,
     `source_id_pattern` = `DOC-(\\d+)`, 원문 = 원본 조항 txt. **첫 판에 거부율 100%**
     (코드 리뷰의 PR# 교훈이 다이얼 설계에 반영돼 결함 0).
...
`demo/` — 발표 대본(`PRESENTATION.md`)과 라이브 클라이맥스 데모(`break-the-gate.sh`).
검수기를 일부러 약화시켜 CI가 빌드를 막는 걸 라이브로 보인다(부작용 0·자동 원복):
...
- [x] **G1+** 의미 채널 — `semantic_core.py`(문자 n-gram 코사인·stdlib). WARN 전용·판정
      권한 0·CI 비차단 정보 스텝. 붕 뜬 근거 귀띔(실측: 발명 수치·뜬구름 칭찬 WARN, 의역 오탐 0).
- [ ] **임베딩/NLI 승급** — 의역·비교방향 왜곡까지 잡으려면 진짜 의미 모델 필요(외부 의존 트레이드오프).

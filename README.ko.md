...
| 품질 채점 하네스 | `wiki_grade.py` — gold 질문셋(질문·기대 1위 노트·기대 근거 문구, 동봉 샘플 12문항)으로 **1위 적중률·top-k recall**, 오류 매설 노트(동봉 5유형: 발명 수치·문구 오인용·출처 없음·Timeline 변조·필수키 누락)로 **게이트 거부율**을 채점. gold 근거 문구의 노트 실재도 검사(fail-closed). JSON 리포트 + `--min-top1`/`--min-reject` 임계 게이트. 동봉 샘플 실측: 적중 12/12·거부 5/5 |
| source-coverage 검수 | `verify_summaries.py --corpus` — Evidence의 수치·인용 문구가 원문(`--source-dir` PDF 추출 txt 우선, 없으면 코퍼스 초록)에 실재하는지 grep 대조(원문 미해결=FAIL·fail-closed), 초록 키포인트 커버율이 임계(기본 0.6) 미달이면 WARN |

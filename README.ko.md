...
| 기능군 | 스크립트 | 역할 |
|---|---|---|
| **wiki** | `wiki_index.py` · `wiki_query.py` · `wiki_promote.py` · `wiki_grade.py` | 색인/감사 · RRF 검색 · 승격 게이트 · 평가 하네스(grade가 query/promote/verify를 import) |
| **corpus** | `classify.py` · `corpus_fetch.py` | 결정론 분류 · arXiv 반입 + `--since` 델타 |
| **verify** | `verify_summaries.py` | 요약 검수 + source-coverage(wiki_grade가 재사용) |
| **team** *(v0.6.0)* | `team_compare.py` | 멀티 LLM 팀 비교 실습 랩 — 팀별 producer/reviewer·결정론 채점 |

...
- `source_grade`(codex#9)는 이 title/abstract의 **출처 등급**. `corpus_fetch`가 arXiv export
  API 원문 verbatim일 때 `api_summary`로 기록한다. `retrieved_at`(YYYY-MM-DD)은 반입 시점 —
  stale·재대조의 기준 시점. 노트로 승격 시 노트 frontmatter의 `source_grade`가 이 등급과
  달라 verbatim 주장을 하면 `verify_summaries`가 경고한다(verbatim은 동일 grade만).
...
type: atomic                  # 선택 — atomic|moc|literature|permanent. type: moc면 그 태그의 MOC로 인식
...
source_grade: api_summary     # 선택(codex#9) — 이 노트 출처 등급. 코퍼스 레코드 grade와 달라
                              #   verbatim 직접인용을 하면 verify_summaries가 경고(동일 grade만)
open_questions: [질문1, 질문2]  # 선택(D4/D5) — 미해소 질문. wiki_index --audit가 집계해
                              #   '다음 사이클 시드'로 재투입(RUNBOOK §3 환류 절)
---
## Compiled Truth
...
- **MOC 자동 제안(agy#5)**: 동일 태그 노트가 5개 이상 모였는데 그 태그를 다루는 MOC 노트
  (`type: moc`)가 없으면 `wiki_index --audit`가 제안(노트 목록·제안 파일명 `<태그>-MOC.md`).
  **제안만** 한다 — MOC 조립은 사람 몫(자동 생성 금지).
- **Open Questions 환류(D4/D5)**: 노트의 `open_questions`를 `wiki_index --audit`가 집계해
  '다음 사이클 시드'로 보여준다. 절차는 RUNBOOK §3 환류 절.
- **파이프라인 상태(agy#11)**: `run_state.py`가 run 사이클 단계(extract→shortlist→summarize→
  verify→organize→delta)를 `_meta/run-state.json`에 기록해 중단·재개를 추적한다.

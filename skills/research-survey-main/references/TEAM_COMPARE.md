...
**1차 E2E(argv 전달)**: claude-producer 팀은 초록만 근거로 4절 요약을 정확히 생성했으나,
codex-producer 팀은 요약 대신 "초록을 보내달라"는 비응답을 냈다. **원인은 모델 성향 차이가
아니라 도구 버그였다**(R1에서 규명·수정): 프롬프트를 명령행 인자(argv)로 넘길 때 **Windows
npm `.CMD` 심이 개행 포함 인자를 첫 개행에서 절단**해, codex가 프롬프트 첫 줄만 받고 초록
본문을 못 받았다(reviewer도 요약 미첨부로 비응답). claude 레그가 정상이었던 건 실행 경로가
달라 절단을 안 탔을 뿐, 프롬프트 내용 문제는 아니었다.
**수정(현재 코드)**: 프롬프트를 **stdin으로 전달**한다(codex exec·claude -p 모두 stdin 수용) —
개행 절단이 원천 차단된다. + reviewer가 비수신·비응답이면 지적 0건을 '무결'로 집계하지 않고
**unchecked(검수 불능)**로 구분 표기한다(위장 무결 차단).
**수정 후 재실행(판별 E2E)**: 같은 2팀·같은 논문으로 다시 돌리니 **codex-producer도 정상
4절 요약을 생성**(인용 실재율 100%·1차의 "초록 달라" 비응답 소멸), 양 팀 reviewer 모두 유효
JSON을 반환해 **실검수(checked)** — 두 요약 다 100% 근거 충실이라 flagged=[]가 정당(비응답
unchecked와 구분됨). 절단 버그가 원천 제거됐음을 실측 확인.
**랩의 교훈은 그대로 유효하다**: 결정론 채점기는 (1차의) producer 비응답을 **거짓 통과시키지
않고** 인용 실재율 '집계 불가'로 표시했다 — 어떤 팀 구성·전달 경로의 실패도 LLM 자평이 아니라
결정론 채점이 드러낸다. 팀 정의 시 각 CLI의 헤드리스 프롬프트 전달을 소규모 실행으로 먼저 확인하라.

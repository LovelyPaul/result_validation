...
> ①추출 ②위키 검색 ③검수 게이트 ④정본 승격 ⑤품질 채점(선택)을 눈으로 확인한다. 진행 톤·
> 산출물 규약은 RUNBOOK 정본을 따른다(각 단계 배너 + 질문 종료·LaTeX 금지·플러그인 폴더 쓰기 금지).
...
- 기대: retrieval 1위 적중률·top-k recall 산출(동봉 샘플 실측: 12/12 적중) + 매설 노트
  거부율 산출 — 각 매설이 **어느 층**(promote lint / source-coverage)에서 잡혔는지 표시.
  JSON 리포트는 `70-analysis/wiki-grade-report.json`.

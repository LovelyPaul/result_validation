...
- week02에서 만든 게이트(승격 lint → source-coverage → gold/매설 채점)를 한 라운드로 보고,
  **같은 결과물을 여러 번** 돌려 판정이 재현되는지 확인한다. `wiki_grade`에 임계를 걸면
  게이트가 된다:
  ```
  wiki_grade … --min-top1 0.9 --min-reject 0.9   # 미달이면 exit 1
  ```
  (research-survey-run 스킬이 실행. gold 무결성 위반은 임계와 무관하게 exit 1 — 기준 오염 차단.)

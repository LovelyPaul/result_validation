...
4. 각 주차 안내는 "이번 주 목표 → 지금 할 일 → 실행(등록 route/스킬 경유) → 산출물 확인 →
   완료 체크 → 다음" 순으로 짧게, 한 번에 한 단계씩. 완료 기준 확인 후 다음 단계로.
   **실행 안내는 반드시 등록 커맨드(`/research-survey <하위명령>`)나 "플러그인 스킬이 실행" 표현으로만**
   한다 — 사용자 워크스페이스 PATH에 없는 내부 스크립트를 bare 이름으로 부르는 지시를 만들지
   않는다(v0.7.1·v0.8.0 R1 교훈).
...
| 실행 가능 명령만 | 안내하는 명령은 실제 등록된 커맨드(`/research-survey <하위명령>`)나 플러그인 스킬 경유 — 사용자 환경에서 안 도는 경로를 지어내지 않는다(v0.7.1·v0.8.0 R1 교훈). |

...
6. **로드맵·마무리**: P2(리랭커·MOC·상태 머신)·P3(vector·GraphRAX·자율 정비) — "도구가 아니라 검수 문화"
...
| 개발 검증 규모 | 릴리스 6회 게이트 전건 수렴(v0.2.x~v0.6.0)·리뷰 라운드 12회+ | 라운드 장부 |
...
| # | 결함 | 놓친 쪽 → 잡은 쪽 | 성격 |
|---|---|---|---|
| 1 | PowerShell `"$c:"` 파싱 오류로 Windows 사전점검 전면 불능 | Claude 리뷰어 2라운드 미탐 → **codex(gpt)가 실행 재현으로 적발** | Windows 셸 계층의 조용한 변형 |
| 2 | 승격 게이트가 workspace 밖 파일도 정본에 씀(경로 미검증) | 동일 라운드 Claude ACCEPT → **codex 적대 재현으로 적발** | 게이트 우회 |
| 3 | npm .CMD 심이 개행 프롬프트를 첫 줄에서 절단 — codex 방향 검수가 "지적 0건"으로 위장 무효 | 최초 "모델 성향" 오진 → **Claude 리뷰어가 판별 실험 2회로 원인 입증** | 위장 무결(silent failure) |
| 공통 교훈 | "문서 코드 블록은 눈으로 읽지 말고 실행 실측하라" / 단일 벤더 구성엔 상관 사각지대가 실재한다 | 양방향(각 벤더가 서로의 미탐을 잡음) | |

→ **team-compare 랩**은 이 경험을 누구나 재현하는 실험으로 상품화한 것: producer/reviewer를 서로 다른 LLM으로 조합(예: codex gpt-5.6-sol × Claude Opus, 역조합)해 동일 입력·동일 결정론 채점기로 검출 차이를 측정한다. 판정은 LLM 자평이 아니라 채점기만 한다.
